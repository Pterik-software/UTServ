...
if (UniSQLUpdateInactivePlan['c_is_active_server']=0) then
  begin
  MessageDlg('Этот план отмечен как неактивный для Вашего тарифного плана на сервере. Свяжитесь с отделом продаж', mtError, [mbOk],0);
  exit;
  end;
if length(EditChangeReason.Text)<10 then
  begin
  MessageDlg('Введите причину активации, не менее 10 символов', mtError, [mbOk],0);
  exit;
  end;
...
UniSQLUpdateInactivePlan.ParamByName('p_log').Value:= 'Изменено пользователем '+DM.CurrentUser+':'+DateTimeToStr(Now()) ;
